...
class Player
{
    ...
    const int TURN_TO_SIMULATE_COUNT = 100;
    ...
    static double GetScore(Chromosome a_chromosome, ShuttleState a_state, List<Line> a_groundLines, int a_flatGroundLineIdx)
    {
        ...
        // Score about distance on X axis to flat ground

        double xdist = Math.Min(Math.Abs(state.Position.X - a_groundLines[a_flatGroundLineIdx].StartX), Math.Abs(state.Position.X - a_groundLines[a_flatGroundLineIdx].EndX));
        double xdistScore = 1 - xdist / ZONE_WIDTH;

        // Score about distance on Y axis to middle of flat ground
